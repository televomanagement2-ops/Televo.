-- =============================================================================
-- Televo — M16 (AC2): Classifica Aura — lifecycle & GDPR
-- =============================================================================
-- docs/aura/classifica.md (Rev. 1) §13.4: le due tabelle di sistema nate in AC1
-- entrano nel ciclo di vita e nei diritti GDPR — nessuna tabella nuova resta
-- senza lifecycle nemmeno per un giorno:
--   1. expire_content v10           → purge degli snapshot di rank oltre i 14
--      giorni (il diff notturno usa solo ieri: minimizzazione) e delle righe di
--      dosaggio del recap oltre i 60 giorni. NESSUN job cron nuovo: viaggia sul
--      cron `expire-content` (5 min) esistente.
--   2. process_account_deletion v9  → art. 17: DELETE immediato degli snapshot
--      dell'utente (la FK cascade coprirebbe comunque l'hard-delete a 30 giorni;
--      qui la cancellazione è subito). Proprietà utile: gli snapshot degli ALTRI
--      non citano l'utente (il rank è un intero personale, senza riferimenti
--      incrociati) — niente da riscrivere altrove. aura_recap_of_week non è
--      per-utente (dosaggio globale): nulla da toccare.
-- La terza gamba di AC2 vive nel repo Edge: gdpr-export v7 (art. 15, sezione
-- aura_rank_snapshots + flag show_in_leaderboard già nel profilo) — in coda
-- deploy owner.
--
-- ⚠️ REGOLA ANTI-REGRESSIONE (vincolo MM1/LM3): entrambe le funzioni copiano il
-- corpo dell'ULTIMA versione in vigore VERBATIM e AGGIUNGONO i soli blocchi M16:
--   · expire_content            ← v9 (20260716120300_live_likes_lifecycle.sql).
--   · process_account_deletion  ← v8 (stessa migrazione).
-- Stessa migrazione = stessa transazione: il cron `expire-content` (5 min) non
-- vede mai uno stato intermedio.

-- =============================================================================
-- 1. expire_content v10 — corpo v9 VERBATIM + purge snapshot/dosaggio in coda.
-- =============================================================================
...
begin
  ...
  -- M16 (AC2): minimizzazione degli snapshot di rank (classifica.md §13.4). Il
  -- diff notturno di aura_rank_daily usa solo lo snapshot più recente PRIMA di
  -- oggi: oltre i 14 giorni la fotografia non serve più a nessuno (il rank vivo
  -- arriva sempre da aura_leaderboard). Le righe di dosaggio del recap
  -- settimanale sono metadato di scheduling: 60 giorni bastano come storia.
  delete from public.aura_rank_snapshots
    where computed_on < current_date - 14;
  delete from public.aura_recap_of_week
    where for_week < current_date - 60;
end;
$$;
...
-- =============================================================================
-- 2. process_account_deletion v9 — corpo v8 VERBATIM + DELETE degli snapshot di
--    rank propri (prima del log_audit finale, che resta la chiusura).
-- =============================================================================
...
begin
  ...
  -- M16 (AC2): gli snapshot di rank dell'utente sono dato personale → via
  -- subito (art. 17; la FK cascade coprirebbe comunque l'hard-delete a 30
  -- giorni). Gli snapshot degli ALTRI non lo citano: il rank è un intero
  -- personale senza riferimenti incrociati — niente da riscrivere altrove.
  delete from public.aura_rank_snapshots where user_id = p_user;

  perform public.log_audit('account_anonymized', 'user', p_user, '{}'::jsonb);
end;
$$;

-- Hardening (M15/LR3, conservato): l'unica via legittima resta la Edge
-- gdpr-delete (service_role) e il cron purge_due_deletions (SECURITY DEFINER);
-- i ruoli client non eseguono mai questa funzione.
