-- =============================================================================
-- Televo — Live M12 (LM3): ciclo di vita v7 (reti di sicurezza cron + GDPR)
-- =============================================================================
-- Quarta wave della Live (docs/live/live.md, Parte II §18 LM3). La Live diventa
-- impossibile da lasciare "orfana" e il diritto all'oblio copre tutto il
-- dominio:
--   1. expire_content v7            → reti di sicurezza: cap durata 8h (QA-1),
--      auto-end della pausa dimenticata a 30 min (QA-2), force-end delle live
--      il cui host non passa più is_active_user() (ban/mute, latenza ≤5 min
--      accettata per l'MVP, §11), purge di commenti/spettatori a 24h dalla
--      fine (§6), minimizzazione delle righe lives a 30 giorni, cintura
--      difensiva sugli eventi mappa live_broadcast rimasti aperti.
--   2. process_account_deletion v7  → art. 17: end + DELETE delle live proprie,
--      rimozione di ogni traccia lasciata su live altrui (§12.14).
-- La terza gamba di LM3 vive nel repo Edge: gdpr-export v5 (art. 15, sezioni
-- live) — in coda deploy owner.
--
-- ⚠️ REGOLA ANTI-REGRESSIONE (vincolo MM1): entrambe le funzioni copiano il
-- corpo v6 LIVE (20260707130000_map_legacy_out.sql) VERBATIM e AGGIUNGONO i
-- soli blocchi live — stessa migrazione = stessa transazione: il cron
-- `expire-content` (5 min) non vede mai uno stato intermedio. NESSUN job cron
-- nuovo: la cadenza è quella esistente.
--
-- Scelte di design (live.md §15.5):
--  · Il force-end passa dall'UPDATE di stato: la macchina a stati resta l'unico
--    arbitro (lives_before_write valorizza ended_at) e gli after-trigger di
--    dominio girano da soli — chiusura badge mappa → Echo 3h (LM1) e premio
--    Aura se qualificata (LM2). Via unica, zero logica duplicata.
--  · I force-end NON fanno fan-out live_ended: per i client vale
--    snapshot-as-truth (lives_feed a mount/foreground + revalidation 60s),
--    scelta consapevole del piano annotata in end_live v2.
--  · Le live finite spariscono (§0.2: nessun archivio): commenti e spettatori
--    a 24h (finestra di moderazione: gli excerpt dei segnalati sopravvivono in
--    moderation_queue), la riga lives a 30 giorni (minimizzazione; live_hosts
--    cascade, map_events.live_id va a NULL da sé — FK di LM1).

-- =============================================================================
-- 1. expire_content v7 — corpo v6 VERBATIM + blocchi live IN CODA.
-- =============================================================================
...
-- =============================================================================
-- 2. process_account_deletion v7 — corpo v6 VERBATIM + blocchi live (prima del
--    log_audit finale, che resta la chiusura della funzione).
-- =============================================================================
